...
function plotElevation(results) {
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    if (i > smoothing - 1) {
      subGrade = calcGrade(
        elevations[i].elevation - elevations[i - smoothing].elevation,
        subDistance
      );
    } else {
      subGrade = calcGrade(
...
function addMarker(latlng, doQuery) {
  if (markers.length < 10) {
    ...
    if (markers.length == 1 && showInfo == true) {
    ...
    if (status == google.maps.DirectionsStatus.OK) {
      ...
      if (measurement == "imperial") {
        ...
        document.getElementById("run").innerHTML =
          (distance / feet_to_mile).toFixed(2) + " mi";
      } else if (measurement == "metric") {
        document.getElementById("run").innerHTML =
          (distance / meter_to_km).toFixed(2) + " km";
      }
...
function loadExample(examples) {
  ...
  for (var i = 0; i < examples[0].latlngs.length; i++) {
    var latlng = new google.maps.LatLng(
      examples[0].latlngs[i][0],
      examples[0].latlngs[i][1]
    );
